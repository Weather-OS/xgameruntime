...
{
    virtual ~ITaskQueuePort() = default;
...
{
    virtual ~ITaskQueuePortContext() = default;
...
public:
    ...
    XTaskQueuePortHandle WINAPI GetHandle() override { return &m_header; }

    HRESULT WINAPI QueueItem(
        ITaskQueuePortContext* portContext,
        uint32_t waitMs,
        void* callbackContext,
        XTaskQueueCallback* callback) override;
    ...
    bool WINAPI IsEmpty() override;

    void WINAPI WaitForUnwind() override;
    ...
    void WINAPI SuspendPort() override;
    void WINAPI ResumePort() override;

    void WINAPI SubmitPendingCallbacks() override;
